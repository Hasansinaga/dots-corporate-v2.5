...
import { Customer, CreditCustomer, CollectibilityType } from '@/features/shared/types/customer';
import { NplData } from '@/features/shared/types/npl';
...
const mapCredit = (loan: any): CreditCustomer => ({
  ...
  collectibility: loan?.collectibility as CollectibilityType | undefined, // ⬅️ kunci tipe
});
...
const uniqByCif = <T extends { cif: string }>(arr: (T | null | undefined)[]): T[] => {
  ...
  for (const it of arr) if (it?.cif) m.set(it.cif, it);
...
export class CustomerRepositoryImpl implements CustomerRepository {
  ...
  async fetchSavingsCustomers(offset: number, limit: number): Promise<Customer[]> {
    ...
    const customers: Customer[] = uniqByCif<Customer>(mapped); // ⬅️ generic eksplisit
    return customers;
  }
  ...
  async fetchCreditCustomers(
    filters?: { collectibility?: string[] },
  ): Promise<{ customers: CreditCustomer[]; npl: NplData }> {
    const res = await API.fetchCreditCustomers({ collectibility: filters?.collectibility });

    if (res?.kind !== 'ok') {
      return { customers: [], npl: { amount: 0, percentage: 0 } };
    }

    const loans: CreditCustomer[] = (res.loans ?? []).map(mapCredit); // ⬅️ tegaskan tipe
    const customers: CreditCustomer[] = uniqByCif<CreditCustomer>(loans); // ⬅️ generic eksplisit
    const npl: NplData = mapNpl(res.npl_data);

    return { customers, npl };
  }

  async searchCreditCustomers(
    q: string,
    filters?: { collectibility?: string[] },
  ): Promise<{ customers: CreditCustomer[]; npl: NplData }> {
