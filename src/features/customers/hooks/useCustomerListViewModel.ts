...
import { CustomerRepositoryImpl } from "../../../shared/services/repositories/customer.repo.impl";
import { fetchSavingsCustomers } from "../../../shared/services/usecases/fetchSavingsCustomers";
import { searchSavingsCustomers } from "../../../shared/services/usecases/searchSavingsCustomers";
import { fetchLoans } from "../../../shared/services/usecases/fetchLoans";
import { searchCreditCustomers } from "../../../shared/services/usecases/searchCreditCustomers";
import type { NplData } from "../../../shared/types/npl";
...
export const useCustomerListViewModel = () => {
  ...
  const loadInitial = useCallback(async () => {
    setLoading(true); setList([]); setOffset(0); setHasMore(true);
    try {
      if (mode === "kredit") {
        const { customers, npl } = await fetchLoans(repo)({ collectibility });
        setList(customers); setNpl(npl); setHasMore(false);
      } else {
        const data = await fetchSavingsCustomers(repo)(0, 100);
        setList(data); setHasMore(data.length === 100); setOffset(100);
  ...
  return {
    mode, setMode, query, setQuery, list, loading, hasMore, npl, maxCollect,
    collectibility, setCollectibility, loadMore, search, formatIDR, reload: loadInitial,
  };
};
