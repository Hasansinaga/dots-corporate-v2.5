...
const COLL_COLORS: Record<string, string> = {
  "1": "#4CAF50", "2": "#FFC107", "3": "#FF9800", "4": "#F44336", "5": "#B71C1C",
};
...
export default function CustomerListScreen({ navigation }: any) {
  ...
  const handleRefresh = async () => {
    try { setRefreshing(true); await vm.search(); } finally { setRefreshing(false); }
  };

  const data = useMemo(() => vm.list.map((x) => ({ ...x, balanceFmt: vm.formatIDR(x.balance) })), [vm.list]);
  ...
  const Header = (
    <View>
      ...
      <View style={S.tabs}>
        <TouchableOpacity onPress={() => { vm.setMode("tabungan"); setFilterOpen(false); }} style={[S.tab, vm.mode === "tabungan" && S.tabActive]}>
          <Text style={[S.tabText, vm.mode === "tabungan" && S.tabTextActive]}>Tabungan</Text>
        </TouchableOpacity>

        <TouchableOpacity onPress={() => vm.setMode("kredit")} style={[S.tab, vm.mode === "kredit" && S.tabActive]}>
          <Text style={[S.tabText, vm.mode === "kredit" && S.tabTextActive]}>Kredit</Text>
        </TouchableOpacity>
      </View>
          ...
          {filterOpen && (
            <View style={S.filterPanel}>
              <View style={S.filterHead}>
                ...
                <TouchableOpacity onPress={() => vm.setCollectibility([])} hitSlop={{ top: 8, bottom: 8, left: 8, right: 8 }}>
              ...
              <CollectibilityChips
                ...
                onToggle={(v) => vm.setCollectibility((prev) => prev.includes(v) ? prev.filter((x) => x !== v) : [...prev, v])}
              />
            </View>
          )}
        </>
  ...
  return (
    <SafeAreaView style={S.safe}>
      ...
      <FlatList
        ...
        renderItem={({ item }) => (
          <CustomerCard item={item} rightTop={renderRightTop(item)} onPress={() => navigation.navigate("Details", { item, activeTab: vm.mode })} />
        )}
        ListHeaderComponent={Header}
        ListEmptyComponent={!vm.loading ? EmptyState : null}
        ListFooterComponent={vm.loading ? <ActivityIndicator style={{ padding: 16 }} /> : null}
...
const S = StyleSheet.create({
  ...
  row: {
    ...
    paddingTop: Platform.OS === "ios" ? 4 : 8, paddingBottom: 10, backgroundColor: "#fff",
    shadowColor: "#000", shadowOpacity: 0.05, shadowRadius: 6, shadowOffset: { width: 0, height: 2 }, elevation: 1,
  },
  iconBtn: { padding: 12, borderRadius: 10, borderWidth: 1, borderColor: "#E5E7EB", backgroundColor: "#fff" },
  iconBtnActive: { borderColor: "#0E73E3" },
  tabs: { flexDirection: "row", paddingHorizontal: 16, gap: 8, marginTop: 10, marginBottom: 8 },
  ...
  filterPanel: { marginHorizontal: 16, marginBottom: 8, borderWidth: 1, borderColor: "#E5E7EB", borderRadius: 10, backgroundColor: "#fff", paddingVertical: 8 },
  filterHead: { paddingHorizontal: 12, paddingBottom: 6, flexDirection: "row", alignItems: "center", justifyContent: "space-between" },
  ...
  emptyIcon: { width: 48, height: 48, borderRadius: 24, borderWidth: 1, borderColor: "#E5E7EB", alignItems: "center", justifyContent: "center", marginBottom: 12 },
