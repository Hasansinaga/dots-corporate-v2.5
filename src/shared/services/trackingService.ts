...
let isLocationTracking = false // Flag untuk mencegah multiple tracking
...
async function getAuthHeaders(overrideLjk?: string) {
  ...
  console.log('[tracking] auth hdr -> tenant:', ljk, '| tokenLen:', token ? token.length : 0)
  return headers
}

function geolocationAvailable(): boolean {
  try { return !!Geolocation } catch { return false }
}
...
// PERBAIKAN 1: Improved permission handling dengan error handling yang lebih baik
async function ensurePermission(): Promise<boolean> {
  ...
  return safeAsync(async () => {
    try {
      ...
      // Tambahkan background location permission untuk Android 10+
      const BACKGROUND = (PermissionsAndroid.PERMISSIONS?.ACCESS_BACKGROUND_LOCATION ??
        'android.permission.ACCESS_BACKGROUND_LOCATION') as PermParam
      ...
      if (!hasFine) {
        ...
        const fineGranted: RequestResult = await PermissionsAndroid.request(FINE, {
        ...
        if (fineGranted !== PermissionsAndroid.RESULTS.GRANTED) {
          console.warn('[tracking] Fine location permission denied')
          return false
        }
      }

      // For Android 10+ (API 29+), request background location if needed
      const androidVersion = Platform.Version
      if (typeof androidVersion === 'number' && androidVersion >= 29) {
        try {
          const hasBackground = await PermissionsAndroid.check(BACKGROUND)
          if (!hasBackground) {
            console.log('[tracking] Requesting BACKGROUND location permission...')
            const backgroundGranted: RequestResult = await PermissionsAndroid.request(BACKGROUND, {
      ...
      } else if (typeof androidVersion === 'string') {
        // Handle case where Platform.Version is string
        const versionNumber = parseInt(androidVersion, 10)
        if (!isNaN(versionNumber) && versionNumber >= 29) {
          try {
            const hasBackground = await PermissionsAndroid.check(BACKGROUND)
            if (!hasBackground) {
              console.log('[tracking] Requesting BACKGROUND location permission (string version)...')
              const backgroundGranted: RequestResult = await PermissionsAndroid.request(BACKGROUND, {
                title: 'Izin Lokasi Background',
                message: 'Untuk tracking yang akurat, aplikasi memerlukan akses lokasi di background.',
                buttonPositive: 'OK',
            ...
            console.warn('[tracking] Background permission request failed (string version), continuing:', bgError)
          }
        }
      }
  ...
  }, false, 'ensurePermission')
}

// PERBAIKAN 2: Improved high accuracy check dengan fallback
async function ensureHighAccuracyEnabled(): Promise<boolean> {
  ...
  return safeAsync(async () => {
    try {
      ...
      type Enabler = {
        promptForEnableLocationIfNeeded: (opts: { interval: number; fastInterval: number }) =>
  ...
  }, true, 'ensureHighAccuracyEnabled') // Default true jika gagal
}

// PERBAIKAN 3: GPS check dengan timeout yang lebih pendek dan error handling yang lebih baik
function checkGpsAvailable(): Promise<boolean> {
  ...
  return new Promise((resolve) => {
    ...
    }, 3000) // Kurangi timeout dari 5s ke 3s
    ...
    try {
      Geolocation.getCurrentPosition(
        ...
        { 
          enableHighAccuracy: false, // Gunakan accuracy yang lebih rendah untuk check
          timeout: 2500, 
...
// PERBAIKAN 4: Enhanced location ready check dengan retry mechanism
export async function ensureLocationReady(): Promise<boolean> {
  try {
    ...
    if (Platform.OS !== 'android') {
      console.log('[tracking] iOS platform detected - assuming location ready')
      return true
    }
    ...
    // Step 2: Enable high accuracy (optional)
    ...
    // Step 3: Test GPS availability with retry
...
// Interface untuk response API (unchanged)
...
// PERBAIKAN 5: API check dengan timeout dan better error handling
export async function isLocationTrackingEnabled(overrideLjk?: string): Promise<boolean> {
  return safeAsync(async () => {
    ...
    const tryRequest = async (attempt: string): Promise<boolean> => {
      ...
      try {
        ...
        if (Array.isArray(data)) {
          ...
          configItem = data.find((item: CompanyConfigResponse) => 
            item && 
    ...
    try {
      ...
      console.warn('[tracking] API request failed:', { status, message: error?.message })
  ...
  }, false, 'isLocationTrackingEnabled')
}

// PERBAIKAN 6: Safer location getting dengan memory management
async function getLocationOnceSafe(): Promise<{ lat: number; lng: number; accuracy?: number } | null> {
    ...
    try {
      Geolocation.getCurrentPosition(
        ...
        { 
          ...
          timeout: 7000, 
          maximumAge: 10000 // Allow slightly older location
        }
      )
    } catch (error) {
      clearTimeout(timeout)
...
// PERBAIKAN 7: Enhanced heartbeat dengan rate limiting
...
// PERBAIKAN 8: Enhanced background tracking dengan safeguards
export async function startBackgroundTrackingIfEnabled(overrideLjk?: string): Promise<boolean> {
  try {
    ...
    isLocationTracking = true // Set flag

    // Try native service first (Android)
    if (Platform.OS === 'android') {
      ...
      if (intentLauncher?.startLocationService) {
        try {
          ...
          }, 2000) // Delay initial heartbeat
          
          return true
        } catch (error) {
          console.warn('[tracking] Native service failed, using JS timer:', error)
        }
      }
    }
    ...
    if (BG_TIMER_ID != null) {
      clearInterval(BG_TIMER_ID) // Clear existing
    }

    // Send initial heartbeat with delay
    setTimeout(() => {
      postLocationHeartbeat(ljk, 'BackgroundStart').catch(() => {})
    }, 3000)
    ...
    await AsyncStorage.setItem(TRACKING_FLAG_KEY, '1')
    console.log('[tracking] JS background tracking started')
    ...
    isLocationTracking = false // Reset flag on error
    return false
  }
}

// PERBAIKAN 9: Enhanced stop tracking
export async function stopBackgroundTracking(): Promise<void> {
  console.log('[tracking] === STOPPING BACKGROUND TRACKING ===')
  
  isLocationTracking = false // Reset flag
...
// Login functions remain mostly the same but with enhanced error handling
...
export function startLoginTrackingSafe(overrideLjk?: string): void {
  setTimeout(() => {
    startLoginTrackingIfEnabled(overrideLjk)
      .then((success) => console.log('[tracking] Background login tracking completed:', success))
      .catch((error) => console.error('[tracking] Background login tracking failed:', error))
...
export async function testLocationTrackingAPI(overrideLjk?: string): Promise<{ success: boolean; config?: any; error?: string }> {
  try {
    const ljk = await resolveTenantId(overrideLjk)
    if (!ljk) {
